import { ShopSection } from './EtsyApi';
...
function slugify(str: string) {
  return str
    ...
    .reduce(function(result, ch) {
      return (
        result + (charMap[ch] || ch).replace(/[^\w\s$*_+~.()'"!\-:@]/g, '')
      );
    }, '')
    .trim()
    .replace(/[-\s]+/g, '-')
...
function sectionUri(section: ShopSection) {
  return `/category/${section.shop_section_id}`;
}
...
export { slugify, sectionUri, sanitize };
